...
from DrivePart import GoogleAuthenticate
...
class App():
  ...
  def __init__(self):
    ...
    # SHOULD BE LOADED ONLY IF A FOLDER CONTAINS USP FOLDER and It's Contents, Which are generated by self.initialize()
    self.REPOSITORY_SETTINGS = self.LOAD_REPOSITORY_SETTINGS()

    if(args.update):self.update()

    if(args.googleLogin) : self.startGoogleLogin()
  ...
  def getGDriveService(self):
    for data in GoogleAuthenticate.get_gdrive_service(self.CCODES):
      if self.CCODES["GOOGLE_SERVICE_OBJECT"]==data["code"]:
        return data["data"]
      else:
        output(data)
  ...
  def startGoogleLogin(self):
    for data in GoogleAuthenticate.startLogin(self.CCODES):
      output(data)
